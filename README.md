...
In this repository, you can also find:
  ...
  * expected_results.csv: A CSV file that provides a list of expected results for instances specified by {model.onnx, property.vnnlib}. These results are sourced from VNNCOMP results for the years 2022, 2023, 2024 and 2025. All properties are provided in both VNN-LIB 1.0 and VNN-LIB 2.0.

## Credits

This repository is the result of the work of [Andrea Valentino Ricotti](https://github.com/andyvale) and [Davide Zito](https://github.com/DavideZito) with the University of Genova.
...
## Convert Instances Script

We have included a script, `to_vnnlib2.py`, which converts the original instances from VNNLIB v1.0 to v2.0. The script optionally accepts as input an `instances.csv` file as input (which may be generated by the filter script mentioned above) containing the instances to convert. If this file is not provided, the script searches the directory specified by `--root DIR` (default: `.`) for all subfiles named `instances.csv`. The translated instances will be saved to the directory specified by `-o` or `--output-dir`, in the same structure in which they were found. The script also accepts an optional `--real-mode` parameter, which tells the converter to use the `Real` data type for variables instead of the data types specified in the corresponding ONNX model files.

The script contains an inline dependency specification to enable use by tools like `uv` or `hatch`. To run with a standard python installation or virtual environment, the script requires the following dependencies: `python>=3.12`, `numpy`, `onnx`, `pandas`, `vnnlib`.

In particular, this script removes the old-style `declare-const` and `declare-func` variable declarations, replacing them with the new `declare-network`, `declare-input`, and `declare-output` declarations. In addition, it uses the new variable indexing syntax introduced in VNNLIB v2.0. It also rewrites floating point values specified in scientific notation to a decimal notation supported by the new VNNLIB parser, and replaces integers with corresponding floating point values in arithmetic and comparison expressions that include non-integer variable identifiers.

### Examples

To convert instances to VNNLIB v2.0, simply run:

```bash
python to_vnnlib2.py -o vnnlib2/
# OR
hatch run to_vnnlib2.py -o vnnlib2/
# OR
uv run to_vnnlib2.py -o vnnlib2/
```

The updated benchmarks will be saved in `vnnlib2`.

To only convert the instances in `Benchmarks_fc`, run:

```bash
python to_vnnlib2.py -o vnnlib2/ --root Benchmarks_fc/
```

To force all specifications to use the `Real` data type instead of the model specific data types, run:

```bash
python to_vnnlib2.py -o vnnlib2/ --real-mode
```
